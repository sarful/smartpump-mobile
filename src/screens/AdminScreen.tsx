  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.card}>
        <Text style={styles.title}>Admin Mobile (Phase 1)</Text>
        <Text style={styles.line}>Welcome, {user?.username}</Text>
        <Text style={styles.line}>Role: {user?.role}</Text>
        <Text style={styles.line}>Admin ID: {user?.adminId || user?.id}</Text>
        <Text style={styles.note}>Phase 2 তে full admin dashboard (users, recharge, requests) যুক্ত হবে।</Text>
